...
void Backward(byte value)
{
  ...
  analogWrite(left,0);
  analogWrite(right,255-value/13);
...
void Stop()
{  
  for (int damp = 8; damp >3; damp--) // damping stop. Stop gentlely.
  {
    analogWrite(forward, 255-analogRead((forward)*damp/10));
    analogWrite(backward, analogRead((backward)*damp/10));
    analogWrite(right, 255-analogRead((right)*damp/10));
    analogWrite(left, analogRead((left)*damp/10));
    delay(200);
  }  
...
void getabyte(){
  switch(command) {
  case 'F':
    Backward(Serial.read());// for the robot, F is backward
    break;
  case 'B':
    Forward(Serial.read());// the robot treat B as forward
    break;
  case 'L':
    Left(Serial.read());
    break;
